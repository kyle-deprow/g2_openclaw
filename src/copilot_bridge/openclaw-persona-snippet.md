...
1. **Include specific file paths.** Don't say "the config file" — say `src/gateway/config.py`.
...
4. **Reference existing patterns.** "Follow the same error-handling pattern used in `src/gateway/server.py`".
...
```
task: "Write a Python function `parse_iso_date(raw: str) -> datetime` in src/gateway/utils.py that parses ISO 8601 date strings and returns UTC datetimes. Raise ValueError for invalid input. Include a docstring."
```

### 2. Fix a bug

```
task: "In src/copilot_bridge/src/client.ts, the `runTask` method doesn't respect the timeout parameter — the fetch call has no AbortSignal. Add an AbortController that cancels after `options.timeout` ms."
```

### 3. Add tests

```
task: "Write pytest unit tests for the `validate_token` function in src/gateway/server.py. Cover: valid token, expired token, malformed token, and missing token. Place tests in tests/gateway/test_server.py following existing patterns."
```

### 4. Multi-file refactoring (use verbose)

```
task: "Rename the `CopilotBridge` class to `CopilotClient` across all files in src/copilot_bridge/. Update imports, type references, and test files. Don't change the public plugin API."
```

### 5. Code review

```
task: "Review src/gateway/protocol.py for potential security issues: injection risks, missing input validation, and unsafe deserialization. Return a numbered list of findings with severity and suggested fix."
```
