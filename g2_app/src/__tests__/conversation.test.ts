...
describe('ConversationHistory', () => {
  ...
  describe('appendHistory', () => {
    it('adds owner updates without replacing the existing conversation', () => {
      conv.replayHistory([
      ...
      conv.appendHistory([
        { role: 'assistant', text: 'Astra update', ts: 3000 },
      ]);

      expect(conv.getEntries().map((entry) => entry.text)).toEqual([
        'spoken question', 'spoken answer', 'Astra update',
      ]);
    });

    it('deduplicates repeated polling updates and keeps newest-first formatting', () => {
      conv.replayHistory([{ role: 'user', text: 'spoken question', ts: 1000 }]);
      const update = { role: 'assistant' as const, text: 'Astra update', ts: 3000 };

      conv.appendHistory([update]);
      conv.appendHistory([update]);

      expect(conv.length).toBe(2);
      const transcript = conv.formatReverse(1000);
      expect(transcript.indexOf('Astra update')).toBeLessThan(transcript.indexOf('spoken question'));
    });
  });
