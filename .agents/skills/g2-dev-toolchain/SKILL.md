...
**`permissions`:** New format (CLI 0.1.6+) is an empty array `[]` or array of strings. Legacy object format `{"network": [...]}` may still work but is no longer generated by the CLI template.
...
| `min_app_version` | Semver string — default `"2.0.0"` in CLI template |
| `min_sdk_version` | SDK version string, optional |
| `supported_languages` | Array of language codes (e.g. `["en"]`), optional |
...
| `permissions` | Array (new format, CLI 0.1.6+) or object (legacy format) — optional |
