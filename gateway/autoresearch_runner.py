...
AUTORESEARCH_STATE_SCHEMA_VERSION = 3
    ...
    def from_dict(cls, raw: object) -> AutoresearchState:
        ...
        if "schema_version" not in data:
            raise AutoresearchValidationError(
                ...
                "initialize a schema-v3 state with `gateway-cli autoresearch-init-state`; "
...
def _validate_quantipy_experiment_evidence(
    ...
    expected_run_id = f"autoresearch-i{state.iteration}-{canonical_commit[:12]}"
    ...
    *,
    ...
    instructions: dict[Phase, str] = {
        ...
        Phase.IMPLEMENTATION: (
            ...
            "ALPHA_RESEARCH, perform all platform data preparation before creating or "
            "running the committed quantipy-experiment-v2 package. The implementation "
            ...
            "union into the fixed, sorted manifest panel request; do not put any "
            "quantipy import, network client, provider, SQL, filesystem, or hydration "
            "logic in prepare.py or any other experiment stage. The v2 runtime "
...
def initialize_state(readiness: PlatformReadinessManifest) -> AutoresearchState:
    """Create a pristine v3 campaign state pinned to authoritative readiness."""
    try:
        identity = readiness.require_ready()
    except ValueError as exc:
        raise AutoresearchValidationError(str(exc)) from exc
