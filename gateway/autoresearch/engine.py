    ...
    *,
    ...
    instructions: dict[Phase, str] = {
        ...
        Phase.VERIFICATION: (
            ...
            "86400) for implementation_result.compute_fit.target gpu or mixed, or "
            "timeout_seconds = min(max(3 * projected_model_seconds + pre_model_seconds, 1800), "
            "86400) for cpu or none, where pre_model_seconds is the prior attempt's measured "
            ...
            "default 86400s for gpu or mixed or the default 86400s for cpu or none. Measured "
        ...
        Phase.REVIEW: (
            ...
            "and an explicit finding_disposition of NONE, FIX_REQUIRED, or DECISION_REQUIRED. "
            "Do not infer finding_disposition from issue text or keywords. "
            "NONE is valid only with no FAIL verdict, no critical issue, and no fix request. "
            "FIX_REQUIRED requires a FAIL verdict or critical issue and at least one exact "
            "fix request; "
            "mixed findings use FIX_REQUIRED while any concrete defect remains. "
            "DECISION_REQUIRED requires a critical issue, permits FAIL or CONDITIONAL PASS, and "
            "requires zero fix requests when the issue is an immutable accepted-evidence property, "
            "such as a bootstrap interval spanning zero, fold concentration, or insufficient "
            "persistence. "
            "Changing the hypothesis, pre-registration, accepted evidence, or launching a "
            "materially "
            "new experiment is not a fix request for the current iteration. "
            "DECISION_REQUIRED routes directly to DECISION_LOG on the first review; FIX_REQUIRED "
            "uses the bounded review/fix loop. Only FIX_REQUIRED critical issues route to fixer. "
            "DECISION_REQUIRED routes directly to DECISION_LOG, never fixer. "
        ...
        Phase.FIX_TEST: (
            "Apply a narrow fix against the latest verification failure or a review result "
            "explicitly marked finding_disposition=FIX_REQUIRED. A review result marked "
...
def _verification_handoff_contract(
    ...
    *,
    ...
    return (
        ...
        "TEST_FAILURE may set all three receipts to null when tests failed. "
        "BUG_SIGNAL may set receipts to null only when the run itself failed to "
        "produce trade metrics; when tests_passed=true and trade_count > 0, the "
        "run succeeded and all three provenance receipts are REQUIRED — produce them "
        "from the consensus universe_plan via qp.security_universe_history() and "
        "qp.validate_dynamic_price_coverage() before submitting. Submitting "
        "BUG_SIGNAL with null receipts on a successful run wastes fix rounds. "
        "Never emit a partial receipt chain or a partial PASS.\n"
        f"{scope_gate}"
