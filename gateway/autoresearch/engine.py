    ...
    *,
    ...
    instructions: dict[Phase, str] = {
        ...
        Phase.VERIFICATION: (
            ...
            "implementation_result.compute_fit.target gpu or mixed, or "
            "timeout_seconds = min(max(3 * projected_model_seconds, 900), 21600) for cpu or "
            "none. A first attempt has no projection, so say so in the artifact and use the "
            "default 28800s for gpu or mixed or the default 14400s for cpu or none. The 900s "
            "floor and "
            "3x headroom multiplier are unchanged; measured projected_model_seconds still "
            "drives the timeout, and the ceiling is only a cap, not permission to skip "
            "projection. A timeout kill yields recoverable "
