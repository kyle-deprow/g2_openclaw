...
- **Exact runtime tuple, no minimum-version checks**: OpenClaw `2026.9.2`, `@openclaw/codex` plugin `2026.9.2`, embedded `@openai/codex` `0.153.4`. Verify all three after installing the official exact-version package from the reviewed offline npm cache.
- **Local setup**: Node >= 22; install the reviewed exact package with `openclaw plugins install npm:@openclaw/codex@2026.9.2 --pin --force --accept-capabilities` (use the reviewed offline npm cache; never use an `npm-pack:` tarball); then `openclaw plugins inspect codex --runtime --json`; `openclaw models auth login --provider openai` (add `--device-code` for headless); then `bash scripts/push-openclaw-config.sh`.
...
- **Validation checklist**: `openclaw plugins inspect codex --runtime --json` proves the loaded plugin `2026.9.2` enabled/loaded with embedded `0.153.4`; `openclaw models list --provider openai` lists the selected model; `openclaw models status --plain` shows a usable route; `openclaw gateway health` succeeds; no legacy external coding-agent provider/runtime refs in repo config.
- **Auth-state gotcha**: effective auth state may inherit usage state from the shared store even when the agent has local credentials. Use supported agent-scoped `openclaw models --agent AGENT status --json` to inspect the effective source; JSON is required for source detail and plain output is summary-only. OAuth login success does not prove inference availability, and a generic unusable/cooldown status may be `subscription_limit`. Inspect current provider allowance before recovery.
- **Recovery boundary**: never blanket-call `clearAuthProfileCooldown`, edit auth SQLite state, copy tokens, switch accounts, or add provider/runtime fallback. Native runtime WHAM reconciliation clears only after a fresh provider-available response and credential/generation guards pass. A local-ownership overlay can make that guarded reconciliation a no-op when the block is shared; preserve a protected backup and escalate rather than retrying or treating internal module imports as a public CLI.
- **Keep `agents.defaults.compaction.mode` at `default`**: OpenClaw 2026.9.2 and native Codex own automatic compaction. Do not add a compatibility shim, API-key fallback, or alternate runtime.
