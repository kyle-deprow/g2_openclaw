...
- **Exact runtime tuple, no minimum-version checks**: OpenClaw `2026.8.1`, `@openclaw/codex` plugin `2026.8.1`, embedded `@openai/codex` `0.151.0`. Verify all three after installing the official exact-version package from the reviewed offline npm cache.
- **Local setup**: Node >= 22; install the reviewed exact package with `openclaw plugins install npm:@openclaw/codex@2026.8.1 --pin --force --accept-capabilities` (use the reviewed offline npm cache; never use an `npm-pack:` tarball); then `openclaw plugins inspect codex --runtime --json`; `openclaw models auth login --provider openai` (add `--device-code` for headless); then `bash scripts/push-openclaw-config.sh`.
