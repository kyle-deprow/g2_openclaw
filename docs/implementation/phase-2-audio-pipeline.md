...
- **Description:** Implement PCM accumulation buffer per doc 02 §2.4 and §4. Accept raw bytes, enforce 60-second max duration limit, convert to float32 numpy array for Whisper. Buffer is chunk-size-agnostic (works with both 40-byte hardware chunks and 3200-byte simulator chunks per review §6 Risk #1).
- **Owner:** backend-python
- **Dependencies:** P1.3 (gateway scaffold exists)
- **Complexity:** M
- **Files created:**
  - `src/gateway/audio_buffer.py`
...
- **Acceptance criteria:**
  ...
  - `append(chunk: bytes)` — appends bytes; raises `BufferOverflow` if accumulated bytes exceed `MAX_DURATION × byte_rate` (doc 02 §4.3: 60s × 32000 B/s = 1,920,000 bytes)
  - `to_numpy() -> np.ndarray` — returns float32 array normalized to [-1.0, 1.0] via `np.frombuffer(bytes, dtype=np.int16).astype(np.float32) / 32768.0` (doc 02 §4.4)
...
- **Description:** Implement Whisper integration wrapper per doc 02 §2.2 and §4.5-4.7. Load model at init, expose async `transcribe()` that runs inference in a thread pool executor. Use greedy decoding, VAD filter, per doc 02 §4.6 inference parameters.
- **Owner:** backend-python
- **Dependencies:** P1.3 (gateway scaffold)
- **Complexity:** M
- **Files created:**
  - `src/gateway/transcriber.py`
  - `tests/gateway/test_transcriber.py`
- **Acceptance criteria:**
  - `Transcriber.__init__(model_name, device, compute_type)` — loads `faster-whisper` model (doc 02 §4.5, default `base.en` / `cpu` / `int8`)
  - `async transcribe(audio: np.ndarray, language: str = "en") -> str` — runs `model.transcribe()` in `asyncio.get_event_loop().run_in_executor()` per doc 02 §11 threading model
  - Inference parameters match doc 02 §4.6: `beam_size=1`, `best_of=1`, `temperature=0.0`, `condition_on_previous_text=False`, `vad_filter=True`, `language="en"`
  - Handles empty transcription → raises `TranscriptionError`
  - 30-second timeout via `asyncio.wait_for()` per doc 02 §4.7
...
- **Description:** Extend `server.py` to handle the full audio flow: `start_audio` → create buffer → accumulate binary frames → `stop_audio` → transcribe → send transcription → mock AI response. Implement state transitions per doc 02 §3: `idle → recording → transcribing → thinking → streaming → idle`. Handle all audio error scenarios from doc 02 §4.7 and §7.2 (#1-5).
- **Owner:** backend-python
- **Dependencies:** P1.5 (server skeleton), P2.1 (audio buffer), P2.2 (transcriber)
- **Complexity:** L
- **Files created/modified:**
  - `src/gateway/server.py` (modify — add audio handling)
  - `src/gateway/config.py` (modify — add Whisper config fields)
...
- **Acceptance criteria:**
  ...
  - Loading state: start WS server immediately, load Whisper in background, send `status:loading` to early connections, transition to `idle` when model ready (doc 02 §9, review §3.2)
  - Config extended with `whisper_model`, `whisper_device`, `whisper_compute_type` per doc 02 §6
  - Tests: mock transcriber, verify full state flow, verify error handling for each scenario

### P2.4 — Mic Capture (`audio.ts`)

- **Description:** Implement microphone capture per doc 03 §6. Open/close mic via `bridge.audioControl()`. Forward each `onMicData` PCM chunk immediately as a binary WebSocket frame — zero buffering on the phone. Handle the `Uint8Array` byte-level view correctly (doc 03 §6 PCM Format Clarification — SDK delivers `Uint8Array`, not `Int8Array`).
- **Owner:** g2-development
- **Dependencies:** P1.7 (gateway.ts), P1.9 (main.ts)
- **Complexity:** M
- **Files created:**
  - `src/g2_app/src/audio.ts`
  - `src/g2_app/src/__tests__/audio.test.ts`
- **Acceptance criteria:**
  - `AudioCapture` class with `start(bridge, gateway)` and `stop()` methods per doc 03 §2
  - `start()`: sends `{type:"start_audio", sampleRate:16000, channels:1, sampleWidth:2}` to gateway, then calls `bridge.audioControl(true)`
  - Audio event handler: on `event.audioEvent`, forwards `audioPcm.buffer` (`Uint8Array.buffer` — SDK delivers `Uint8Array`, NOT `Int8Array` per review A.3) via `gateway.send()` immediately — no buffering (doc 03 §6 critical constraint)
  - `stop()`: calls `bridge.audioControl(false)`, sends `{type:"stop_audio"}` to gateway
  - Prerequisite enforced: `audioControl()` only called after `createStartUpPageContainer` (doc 03 §6)
  - Handles both 40-byte (real hardware) and 3200-byte (simulator) chunks transparently

### P2.5 — Input Handler (`input.ts`)

- **Description:** Implement full event routing per doc 03 §7. Handle the `CLICK_EVENT = 0 / undefined` SDK bug. Route taps based on current state (tap-to-toggle model). Handle all event types from `OsEventTypeList`. Implement scroll throttling (300ms cooldown per doc 03 §7).
- **Owner:** g2-development
- **Dependencies:** P1.6 (state.ts), P2.4 (audio.ts)
- **Complexity:** M
- **Files created:**
  - `src/g2_app/src/input.ts`
- **Acceptance criteria:**
  - `InputHandler` class with `setup(bridge)` per doc 03 §2
  - Registers `bridge.onEvenHubEvent()` callback
  - Checks all three event sources: `event.textEvent?.eventType`, `event.listEvent?.eventType`, `event.sysEvent?.eventType` (doc 03 §7)
  ...
  - Tap-to-toggle mapping per doc 03 §7 complete event table:
  ...
  - Unit tests in `src/g2_app/src/__tests__/input.test.ts` covering CLICK_EVENT=0/undefined workaround, tap routing per state, scroll throttling (review F.1)

### P2.6 — Display: Recording + Transcribing States

- **Description:** Add recording (doc 04 §4.2) and transcribing (doc 04 §4.3) layouts to `display.ts`. Recording shows audio level bar with Unicode blocks. Transcribing shows animated dots.
- **Owner:** g2-development
- **Dependencies:** P1.8 (display.ts base)
- **Complexity:** S
- **Files created/modified:**
  - `src/g2_app/src/display.ts` (modify — add `showRecording()`, `showTranscribing()`)
- **Acceptance criteria:**
  - `showRecording()` — 4 containers matching doc 04 §4.2: badge `"● Recording"` in accent (0xC), content `"Listening...\n\n████████░░░░░░░░"`, footer `"Tap to stop"`
  - `showTranscribing()` — 4 containers matching doc 04 §4.3: badge `"● Transcribing"` in accent (0xC), content `"Transcribing audio..."`, footer `"Processing speech"`
  - Recording → full `rebuildPageContainer` on entry and exit
  - Transcribing → `rebuildPageContainer` on entry
  - Animated dots on transcribing via 500ms `textContainerUpgrade` cycle (doc 04 §4.3 notes)
...
- **Complexity:** M
- **Files created/modified:**
  - `src/g2_app/src/main.ts` (modify — integrate audio + input)
- **Acceptance criteria:**
...
- P2.1 (buffer), P2.2 (transcriber), P2.4 (mic capture), P2.5 (input handler) all start immediately in parallel — no interdependencies (review B.3: P2.2 doesn't need P2.1's code, just the numpy array spec from doc 02 §4.4)
...
- [x] All 5 audio error scenarios (doc 02 §4.7) produce correct error frames and recovery
