...
- **Description:** Define all JSON frame types (inbound + outbound) as TypedDicts per [gateway.md](../design/gateway.md) §2.6 and [architecture.md](../design/architecture.md) Appendix A. Include `parse_text_frame()` and `serialize()` helpers. Resolve all naming conflicts identified in review §2.2: use `message` for TextMessage payload, `detail` for ErrorFrame, include `version` in ConnectedFrame.
...
- **Dependencies:** None (can reference [architecture.md](../design/architecture.md) Appendix A independently of P1.1)
- **Complexity:** S
- **Files created:**
  - `g2_app/src/protocol.ts`
- **Acceptance criteria:**
  ...
  - All field names match P1.1 exactly (coordinated via [architecture.md](../design/architecture.md) Appendix A)

### P1.2.5 — Markdown Stripping Utility (moved from P3.5 per review C.2)

- **Description:** Implement `stripMarkdown(text: string): string` per [display-layouts.md](../design/display-layouts.md) §6. Zero-dependency pure utility. Available from Phase 1 for mock response testing — makes Phase 1 integration more realistic since real LLM responses contain markdown.
- **Owner:** g2-development
- **Dependencies:** None
- **Complexity:** S
- **Files created:**
  - `g2_app/src/utils.ts`
  - `g2_app/src/__tests__/utils.test.ts`
- **Acceptance criteria:**
...
- **Description:** Create the Gateway Python project with `pyproject.toml`, `config.py`, and basic project structure. Config loads from env vars / `.env` per [gateway.md](../design/gateway.md) §6. For Phase 1, only `GATEWAY_HOST`, `GATEWAY_PORT`, and `GATEWAY_TOKEN` are needed (no Whisper or OpenClaw config yet).
- **Owner:** backend-python
- **Dependencies:** None
- **Complexity:** S
- **Files created:**
...
- **Description:** Create the G2 app project: `package.json`, `tsconfig.json`, `vite.config.ts`, `app.json` manifest, `index.html` entry point. Configure Vite for single-bundle output per [g2-app.md](../design/g2-app.md) §9.
- **Owner:** g2-development
- **Dependencies:** None
- **Complexity:** S
- **Files created:**
  - `g2_app/package.json`
  - `g2_app/tsconfig.json`
  - `g2_app/vite.config.ts`
  - `g2_app/app.json`
  - `g2_app/index.html`
- **Acceptance criteria:**
  - `package.json` lists `@evenrealities/even_hub_sdk` dependency and `vite` + `typescript` dev deps
  - `vite.config.ts` produces single JS bundle (`inlineDynamicImports: true`) per [g2-app.md](../design/g2-app.md) §9
...
- **Description:** Implement the app-side state machine per [g2-app.md](../design/g2-app.md) §4. Track current state, validate transitions, emit state change events. The `displaying` state ignores `status:"idle"` from Gateway (review §3.3).
- **Owner:** g2-development
...
- **Acceptance criteria:**
  ...
  - `StateMachine` class with `transition(newState)`, `current`, `onChange(cb)` per [g2-app.md](../design/g2-app.md) §2
  ...
  - Unit tests in `g2_app/src/__tests__/state.test.ts` covering all valid/invalid transitions (review F.1)

### P1.7 — App WebSocket Client (`gateway.ts`)

- **Description:** Implement the Gateway WebSocket client per [g2-app.md](../design/g2-app.md) §5. Connect to `ws://<PC_IP>:8765?token=<secret>`, send binary/JSON frames, receive and parse JSON frames, handle auto-reconnect with exponential backoff. URL resolved from hash > query > localStorage > build-time env per [g2-app.md](../design/g2-app.md) §8.
- **Owner:** g2-development
- **Dependencies:** P1.2 (protocol.ts), P1.4 (project scaffold)
- **Complexity:** M
- **Files created:**
  - `g2_app/src/gateway.ts`
- **Acceptance criteria:**
  ...
  - Auto-reconnect: 1s → 2s → 4s → 8s → 16s → 30s max backoff per [g2-app.md](../design/g2-app.md) §5
  - Emits `'connected'`, `'disconnected'`, `'reconnecting'` events
  - URL resolution from hash → query → localStorage → env per [g2-app.md](../design/g2-app.md) §8
...
- **Description:** Implement the Display class with layouts for 3 states: idle ([display-layouts.md](../design/display-layouts.md) §4.1), streaming ([display-layouts.md](../design/display-layouts.md) §4.5), and response complete/displaying ([display-layouts.md](../design/display-layouts.md) §4.6). Use the `text()` helper and `rebuild()` wrapper from [display-layouts.md](../design/display-layouts.md) §7.1. Implement `textContainerUpgrade` for delta appending per [display-layouts.md](../design/display-layouts.md) §7.3.
- **Owner:** g2-development
- **Dependencies:** P1.2 (protocol.ts), P1.4 (project scaffold)
- **Complexity:** M
- **Files created:**
  - `g2_app/src/display.ts`
- **Acceptance criteria:**
  - `text()` helper builds `TextContainerProperty` with all fields including `borderRdaius` typo ([display-layouts.md](../design/display-layouts.md) §7.1)
  ...
  - `showIdle()` — 4 containers matching [display-layouts.md](../design/display-layouts.md) §4.1 spec exactly (positions, font sizes, colors)
  - `showStreaming(query: string)` — 4 containers matching [display-layouts.md](../design/display-layouts.md) §4.5 spec; content container scrollable
  - `showResponse(query: string, response: string)` — 4 containers matching [display-layouts.md](../design/display-layouts.md) §4.6 spec
  - `appendDelta(delta: string)` — uses `textContainerUpgrade` to append text at correct offset ([display-layouts.md](../design/display-layouts.md) §7.3)
...
- **Files created:**
  ...
  - `gateway/README.md` (quick-start instructions)
- **Acceptance criteria:**
  - Gateway starts: `uv run python -m gateway.server`
  - App builds and serves: `cd g2_app && npx vite --host 0.0.0.0`
...
- P1.1 and P1.2 run fully in parallel (both derive from [architecture.md](../design/architecture.md) Appendix A)
...
- [x] `cd g2_app && npx tsc --noEmit` — no type errors
- [x] `cd g2_app && npx vite build` — produces working bundle
- [ ] `cd g2_app && evenhub pack app.json dist -o openclaw.ehpk` — produces valid `.ehpk` package *(Phase 4 P4.12)*
- [x] Gateway starts with `uv run python -m gateway.server` from `gateway/`
