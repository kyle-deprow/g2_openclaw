...
**Date:** 2026-02-22  
**Documents Reviewed:** Design docs (architecture.md, gateway.md, g2-app.md, display-layouts.md, architecture-review.md), Implementation phases 1-4, Skills (g2-display-ui, g2-sdk-bridge, g2-events-input, g2-dev-toolchain), copilot-instructions.md
...
P4.8 (Page 1 Detail View) and the entire page flip interaction model rely on `bridge.setPageFlip(0|1)`. This method appears in design docs [architecture.md](../../design/architecture.md) §5, [g2-app.md](../../design/g2-app.md), and [display-layouts.md](../../design/display-layouts.md) §7.4, but is **NOT listed in the g2-sdk-bridge skill's complete public method reference** (15 methods documented, none named `setPageFlip`).
...
- All page 1 container specs from [display-layouts.md](../../design/display-layouts.md) §5
...
Design [g2-app.md](../../design/g2-app.md) §6 and P2.4 reference `Int8Array` for PCM chunks from `onMicData`. Both the g2-sdk-bridge skill (`AudioEventPayload.audioPcm: Uint8Array`) and the g2-events-input skill explicitly state the type is `Uint8Array`.
...
The g2-sdk-bridge skill's data model table lists the TextContainerProperty field as `content`. The g2-display-ui skill examples use `textContent`. [display-layouts.md](../../design/display-layouts.md) §7.1 uses `content`. The SDK's `pickLoose()` normalization may accept both, but relying on undocumented magic for a critical field is risky.
...
P2.6 (recording + transcribing display states) is listed as depending on P2.4 (audio.ts) with rationale "needs to know audio.ts interface for level bar updates." But the recording display layout ([display-layouts.md](../../design/display-layouts.md) §4.2) renders a static Unicode block bar (`████████░░░░░░░░`). The audio level bar updates mentioned in the notes ("updated via `textContainerUpgrade` from PCM RMS") are a nice-to-have feature not actually specified anywhere in the acceptance criteria for P2.6.
...
P2.2 (transcriber) depends on P2.1 (audio buffer) "for numpy array format." But the numpy array format is already fully specified in [gateway.md](../../design/gateway.md) §4.4: `float32, [-1.0, 1.0], from np.int16 / 32768.0`. P2.2 doesn't need P2.1's code — it just needs to accept `np.ndarray` input.
...
P4.5 (error display), P4.6 (disconnected display), and P4.7 (loading display) are all "add a `show<State>()` method to display.ts with 4 containers matching [display-layouts.md](../../design/display-layouts.md) spec." Combined, they're ~15 lines of container definitions each. The overhead of three separate agent calls (context loading, file reading, verification) far exceeds the marginal work.
...
The P4.12 acceptance criteria should include: "Manifest validated against g2-dev-toolchain skill schema." Currently it references [g2-app.md](../../design/g2-app.md) §9 which has the wrong field names.
...
Review [architecture-review.md](architecture-review.md) §7.4 flags this. When Whisper produces bad transcriptions, there's no way to inspect the PCM data that was sent. A `DEBUG_AUDIO_DUMP=1` env var that writes each recording's PCM to `/tmp/g2_audio_<timestamp>.raw` would be invaluable for debugging.
...
[architecture.md](../../design/architecture.md) §7 recommends WSS for sensitive audio. No plan task implements TLS. For v1 on a home LAN this is acceptable, but should be documented as a known limitation.
...
[display-layouts.md](../../design/display-layouts.md) §8 transition table notes: "transitioning from displaying to recording requires two taps: first tap dismisses (→ idle), second tap starts recording (→ recording)." This is by design but creates friction for sequential queries. Review [architecture-review.md](architecture-review.md) §7.6 flags this.
