...
**Date:** 2026-02-22
**Documents Reviewed:** 01-architecture-overview.md, 02-pc-gateway-design.md, 03-g2-app-design.md, 04-display-layouts.md, plus openclaw_research/ (8 files)
...
**Docs affected:** 01 §5, 03 §7, 04 §8

The entire design assumes hold-to-record, release-to-send. Doc 01 §5 shows "HOLD button → open mic, RELEASE btn → send stop_audio." Doc 03 §6 labels the flow as "User holds ring button" → "User releases ring button."

But doc 03 §7 buries the truth: **"The G2 SDK does not provide distinct HOLD/RELEASE events."** The SDK fires `CLICK_EVENT` on tap — there is no hold detection, no key-down, no key-up. The doc then pivots to a tap-to-toggle model under "Hold vs Tap Detection" but never reconciles this with the rest of the architecture.
...
**Fix:** Globally replace the hold-to-record model with tap-to-toggle. Update docs 01 §4 step 6, 01 §5 interaction map, 02 §3 transition table, 03 §4 state diagram, 03 §6 step-by-step, 03 §7 event table, and 04 §8 transition table.

### 2.2 Protocol Field Name Conflicts Between Docs

Three field name disagreements would cause immediate JSON parse failures:

| Frame | Doc 01 (Appendix A) | Doc 02 (protocol.py) | Which is Correct? |
|---|---|---|---|
...
**Fix:** Canonicalize in protocol.py (doc 02) and update doc 01 Appendix A to match. Suggested: use `message` for user text input, `detail` for error details, and include `version` in the connected frame.

### 2.3 OpenClaw Lifecycle Event Field Name May Be Wrong

Doc 02 §5.6 and doc 01 Appendix B parse lifecycle events as:
```json
...
**Fix:** Verify against the actual OpenClaw source code. If the field is `phase`, update doc 01 Appendix B, doc 02 §5.6 event parsing table, and the `openclaw_client.py` design.

### 2.4 Authentication Mechanism is Contradictory

Two different auth mechanisms are described:

- Doc 01 Appendix A: `ws://<PC_IP>:8765?token=<shared_secret>` (query parameter)
- Doc 02 §9 Step 5: "Phone sends `{type:"auth", token:"..."}` as its first message"

These are mutually exclusive. The `auth` frame type doesn't appear in the Phone→Gateway frame table (doc 01 Appendix A), and there's no `auth` class in protocol.py (doc 02 §2.6).
...
Doc 01 Appendix A defines `{type:"cancel"}` as a Phone→Gateway frame. It is never mentioned anywhere in doc 02:
...
Doc 02 §9 shows Whisper model loading takes 5–30s at startup. If the phone connects during this window, the Gateway sends `{type:"connected"}` but Whisper cannot transcribe. If the user immediately starts recording:
...
Doc 03 §4 and doc 04 §8 define a `displaying` (response complete) state on the app. The app enters this state when it receives `{type:"end"}`, then separately receives `{type:"status", status:"idle"}`. But the app's state machine transitions on `end` frame, not on status frames.
...
**Fix:** Explicitly document that the app does NOT blindly transition on every `status` frame. The `displaying` state ignores `status:"idle"`. The app transitions to idle only on user input (tap to dismiss) or on starting a new recording. Document this exemption in doc 03 §4 and doc 04 §8.

### 3.4 2000-Character Display Limit Will Be Hit Routinely

Doc 03 §3.4 notes: "If accumulated text exceeds 1000 chars (the `rebuildPageContainer` limit), switch to `textContainerUpgrade` which allows 2000 chars."
...
**Fix:** Design a truncation strategy. Options: (a) truncate to 2000 chars with a "... [truncated, double-tap for more]" indicator and put the overflow on page 1; (b) strip markdown/whitespace to compress; (c) use a `textContainerUpgrade` reset technique (rebuild with the tail of the response). Document the chosen approach in doc 03 §3.4 and doc 04 §4.5.

### 3.5 Streaming Response Starts Before Layout Swap

Doc 02 §3 transition table says: "thinking → streaming" triggers on "first assistant delta." Doc 04 §8 says the transition from thinking to streaming uses `rebuildPageContainer` (full layout swap), then subsequent deltas use `textContainerUpgrade`.
...
**Fix:** Buffer deltas during the thinking→streaming layout transition. The app should set a `layoutPending` flag, collect deltas in a local array, and flush them all via `textContainerUpgrade` once `rebuildPageContainer` resolves. Document this in doc 03 §3.4 and the streaming strategy section.

### 3.6 PCM Format Ambiguity: Int8Array vs S16LE

Doc 03 §6 says onMicData delivers `Int8Array` chunks, and the PCM format reference says "PCM S16LE (signed 16-bit little-endian)." Doc 02 §4.2 says "Sample width: 2 bytes (16-bit), Signed little-endian (PCM S16LE)."
...
**Fix:** Add a note in doc 03 §6 PCM Format Reference: "The `Int8Array` from `onMicData` is a byte-level view of 16-bit PCM S16LE data — each sample spans 2 consecutive bytes. Send the raw bytes as-is; the PC Gateway assembles them into a proper WAV."

### 3.7 No Timeout for Full Request Cycle

Doc 02 specifies a 30s timeout for Whisper inference, and the OpenClaw research notes show OpenClaw's default timeout is 600s. But no end-to-end timeout is defined for the full voice-to-response cycle as seen by the phone.
...
### 4.1 Display Layout Inconsistency Between Docs 03 and 04

Doc 03 §3.1 uses a different layout grid than doc 04 §3:

| Property | Doc 03 Idle | Doc 04 Idle |
|---|---|---|
...
Doc 04 introduces a consistent 4-container layout with a header badge that doc 03 doesn't use. Doc 04 is clearly the more mature design (it adds the badge for state indication and uses a proper margin system).

**Fix:** Replace all layout specs in doc 03 §3.1–3.5 with references to doc 04. "See 04-display-layouts.md for pixel-level specifications." Remove the inline layout tables from doc 03 to avoid drift.

### 4.2 `borderRdaius` Typo in SDK Preserved Without Comment

Doc 04 §7.1 preserves the SDK's typo (`borderRdaius` instead of `borderRadius`) without noting it. The comment `// SDK typo preserved` is only visible if someone reads the code example carefully.
...
- Doc 01: references `OPENCLAW_GATEWAY_TOKEN` (the OpenClaw env var name)
- Doc 02 §6: defines `OPENCLAW_TOKEN` as the Gateway config variable
...
**Fix:** Rename to `OPENCLAW_GATEWAY_TOKEN` (for OpenClaw auth) and `GATEWAY_TOKEN` (for phone auth), matching both the established OpenClaw convention and the Gateway's own naming. Update doc 02 §6 table.

### 4.4 WAV Conversion May Be Unnecessary

Doc 02 §4.4 builds a 44-byte WAV header for Whisper consumption. However, `faster-whisper` can accept raw numpy arrays or file paths. Constructing a WAV blob in memory, then having `faster-whisper` immediately decode it back to samples, is a pointless round-trip.
...
Doc 03 §9 uses Vite, which is correct for an EvenHub WebView app. No issue here, just confirming this is fine — EvenHub apps are plain web apps packaged as `.ehpk`. Vite's single-bundle output matches the WebView's requirements.

### 4.6 `CLICK_EVENT = 0` Workaround Could Mask Real Bugs

Doc 03 §7 handles the SDK bug by checking `eventType === undefined`. This means *any* undefined event type will be treated as a click. If the SDK adds new event types that the app doesn't handle, they'll also fall into the click handler.
...
| `session_reset` | Phone→GW | Request a new OpenClaw session (mentioned in doc 02 §5.5 as future extension but should be in v1) |
| `tool_use` | GW→Phone | OpenClaw tool events (`stream:"tool"`) — doc 02 §5.6 says "optionally forward" with no design |
...
TCP guarantees in-order delivery, and WebSocket inherits this. So yes, but only if the Gateway sends frames in the correct order. Doc 02's transition table implies this ordering but never explicitly states it as a protocol invariant.

**Fix:** Add a "Protocol Invariants" section to doc 01 Appendix A that states frame ordering guarantees explicitly.
...
| 2 | BLE bandwidth saturation from frequent `setLayout` / `textContainerUpgrade` calls during streaming | **Medium** | **High** | Implement throttling (doc 04 §4.5 says 15/sec but provides no mechanism). Batch 2–3 deltas per display update. Measure actual BLE throughput on hardware. |
...
| 9 | OpenClaw sends markdown-formatted responses (`**bold**`, `[links](url)`, `` `code` ``) that render as raw syntax on the greyscale display | **High** | **Low** | Doc 04 §6 mentions markdown stripping but provides no implementation. Write a `stripMarkdown()` utility and apply to every delta before display. |
...
- Can the user start a new recording while viewing a previous response (`displaying` state)? Doc 03 §7 says yes (ring hold from `displaying` state), but **doc 04 §8 transition table doesn't show a `displaying → recording` transition directly**. It shows `response complete → idle` via tap, then `idle → recording`. This means the user must dismiss first, then record — two gestures instead of one. The display will flash through idle unnecessarily.
...
4. **Display: idle + streaming + response complete** — Just these three doc 04 layouts. Prove that `rebuildPageContainer` and `textContainerUpgrade` work on the simulator.
...
14. **Error handling** — all 13 error scenarios from doc 02 §7.2.
