# Even Realities G2 — Comprehensive Developer Reference Guide

> **Source:** Compiled from [nickustinov/even-g2-notes](https://github.com/nickustinov/even-g2-notes) (commit `177b5aa`, Feb 2026).
> **Disclaimer:** This document is based on independent research and reverse-engineering of the public SDK. It is not official documentation, may contain inaccuracies, and is updated as new information becomes available. The original author is not affiliated with Even Realities.
...
- [2. Architecture](#2-architecture)
  - [2.1 Communication Flow](#21-communication-flow)
  - [2.2 Two-Way Communication](#22-two-way-communication)
  - [2.3 Implications for App Design](#23-implications-for-app-design)
- [3. G2 Hardware Specifications](#3-g2-hardware-specifications)
- [4. Display and UI System](#4-display-and-ui-system)
  - [4.1 Canvas](#41-canvas)
  - [4.2 Container Model](#42-container-model)
  - [4.3 Shared Container Properties](#43-shared-container-properties)
  - [4.4 Border and Decoration](#44-border-and-decoration)
  - [4.5 Text Containers (`TextContainerProperty`)](#45-text-containers-textcontainerproperty)
  - [4.6 List Containers (`ListContainerProperty`)](#46-list-containers-listcontainerproperty)
  - [4.7 Image Containers (`ImageContainerProperty`)](#47-image-containers-imagecontainerproperty)
- [5. Input Events](#5-input-events)
  - [5.1 Event Types (`OsEventTypeList`)](#51-event-types-oseventtypelist)
  - [5.2 Event Delivery](#52-event-delivery)
  - [5.3 Event Quirks (Critical for Real-World Apps)](#53-event-quirks-critical-for-real-world-apps)
- [6. Page Lifecycle](#6-page-lifecycle)
  - [6.1 `createStartUpPageContainer`](#61-createstartuppagecontainer)
  - [6.2 `rebuildPageContainer`](#62-rebuildpagecontainer)
  - [6.3 `textContainerUpgrade`](#63-textcontainerupgrade)
  - [6.4 `updateImageRawData`](#64-updateimagerawdata)
  - [6.5 `shutDownPageContainer`](#65-shutdownpagecontainer)
  - [6.6 `callEvenApp` (Generic Escape Hatch)](#66-callevenapp-generic-escape-hatch)
- [7. Audio](#7-audio)
- [8. Device Info](#8-device-info)
- [9. User Info](#9-user-info)
- [10. Local Storage](#10-local-storage)
- [11. What the SDK Does NOT Expose](#11-what-the-sdk-does-not-expose)
- [12. Error Codes](#12-error-codes)
  - [12.1 Page Creation (`StartUpPageCreateResult`)](#121-page-creation-startuppagecreateresult)
  - [12.2 Page Rebuild](#122-page-rebuild)
  - [12.3 Text Upgrade](#123-text-upgrade)
  - [12.4 Image Update (`ImageRawDataUpdateResult`)](#124-image-update-imagerawdataupdateresult)
  - [12.5 Shutdown](#125-shutdown)
- [13. SDK JSON Compatibility](#13-sdk-json-compatibility)
- [14. UI Patterns from Real Apps](#14-ui-patterns-from-real-apps)
  - [14.1 Fake "Buttons" with Text Cursor](#141-fake-buttons-with-text-cursor)
  - [14.2 Selection Highlight with Text Borders](#142-selection-highlight-with-text-borders)
  - [14.3 Multi-Slot Text Layout](#143-multi-slot-text-layout)
  - [14.4 Progress Bars with Unicode](#144-progress-bars-with-unicode)
  - [14.5 Event Capture for Image-Based Apps](#145-event-capture-for-image-based-apps)
  - [14.6 Page Flipping for Long Text](#146-page-flipping-for-long-text)
- [15. Browser UI Component Library](#15-browser-ui-component-library)
  - [15.1 Entry Points](#151-entry-points)
  - [15.2 Components](#152-components)
  - [15.3 Icons (90+)](#153-icons-90)
  - [15.4 Design Tokens (CSS Custom Properties)](#154-design-tokens-css-custom-properties)
  - [15.5 Utility](#155-utility)
- [16. Even-Dev Simulator Environment](#16-even-dev-simulator-environment)
  - [16.1 Running Apps](#161-running-apps)
  - [16.2 App Structure](#162-app-structure)
  - [16.3 Even Hub CLI (`@evenrealities/evenhub-cli`)](#163-even-hub-cli-evenrealitiesevenhub-cli)
  - [16.4 Packaging and Deployment](#164-packaging-and-deployment)
- [17. Example Apps](#17-example-apps)
...
### 2.3 Implications for App Design
...
## 3. G2 Hardware Specifications

| Feature | Detail |
|---|---|
| **Display** | Dual micro-LED displays (green), synced via physical FPC (not wireless) |
| **Resolution** | 576×288 pixel canvas per eye |
| **Color depth** | 4-bit greyscale – 16 shades of green. White pixels appear as bright green on the display; black pixels are off |
| **Connectivity** | BLE 5.x – ~28m real-world range, ~9dB more power than G1 |
| **Microphone** | Yes (accessible via SDK) |
| **Input** | Touch gestures on temple tips |
...
## 4. Display and UI System

### 4.1 Canvas

- **Resolution:** 576×288 pixels
- **Coordinate system:** Origin (0, 0) at top-left, X right, Y down
- **Display type:** Green micro-LED display
- **Color handling:** All colours are converted to 4-bit greyscale (16 levels of green) by the host app before transmission
- **Border colour ranges:** Vary slightly by container type — 0–15 for list containers, 0–16 for text containers
...
### 4.3 Shared Container Properties
...
**Limitations:**
- No background colour property
- No fill colour
- The only visual decoration is the border

### 4.5 Text Containers (`TextContainerProperty`)
...
|---|---|
| `createStartUpPageContainer` | 1000 characters |
| `textContainerUpgrade` | 2000 characters |
| `rebuildPageContainer` | 1000 characters (same as startup) |
...
### 4.6 List Containers (`ListContainerProperty`)
...
List containers take over scroll handling. If you have a list on screen, scroll events arrive as `listEvent` (not `textEvent`/`sysEvent`), and the device moves the selection highlight automatically. You only need to respond to click/double-click.

### 4.7 Image Containers (`ImageContainerProperty`)
...
### 5.1 Event Types (`OsEventTypeList`)
...
### 5.3 Event Quirks (Critical for Real-World Apps)
...
### 6.1 `createStartUpPageContainer`
...
### 6.6 `callEvenApp` (Generic Escape Hatch)
...
```typescript
const unsubscribe = bridge.onDeviceStatusChanged(callback)
// Returns an unsubscribe function
```

### `DeviceInfo.updateStatus(status)`

Updates the device's status in-place, but only when `status.sn === device.sn` (the serial numbers must match; mismatched updates are silently ignored).

### Helper Methods

**On `DeviceInfo`:** `isGlasses()`, `isRing()`

**On `DeviceStatus`:** `isConnected()`, `isConnecting()`, `isDisconnected()`, `isConnectionFailed()`, `isNone()`

### Serialisation Helpers (All Models)

- `toJson()` – instance method
- `fromJson(json)` – static method
- `createDefault()` – static method (on `UserInfo` and `DeviceStatus`)
...
## 11. What the SDK Does NOT Expose
...
### 12.1 Page Creation (`StartUpPageCreateResult`)
...
### 12.4 Image Update (`ImageRawDataUpdateResult`)
...
## 14. UI Patterns from Real Apps

### 14.1 Fake "Buttons" with Text Cursor
...
### 14.2 Selection Highlight with Text Borders
...
### 14.4 Progress Bars with Unicode
...
### 14.5 Event Capture for Image-Based Apps
...
**Important notes:**
- Image containers are limited to 200×100 px max, so they cannot cover the full 576×288 canvas
- Centre the image container on screen (`xPosition = (576−200)/2 = 188`, `yPosition = (288−100)/2 = 94`) and use the text container's text or a black background to fill the rest
- This gives you click, double-click, scroll-top and scroll-bottom events while the image is visible
- Events arrive as `textEvent` (not `listEvent`), so adjust your event handler accordingly

### 14.6 Page Flipping for Long Text
...
## 15. Browser UI Component Library
...
### 15.4 Design Tokens (CSS Custom Properties)
...
## 16. Even-Dev Simulator Environment
...
At minimum, the Even Hub SDK and Vite:
...
Import the even-realities-ui stylesheet for consistent browser UI, then add your own styles:
...
- **Use `@jappyjan/even-realities-ui`** for the browser settings page – Card, Button, Input, Text components give a consistent look across apps.
...
### 16.3 Even Hub CLI (`@evenrealities/evenhub-cli`)
...
NPM: <https://www.npmjs.com/package/@evenrealities/even_hub_sdk>
...
## Appendix B: Quick Reference — Key Constants and Limits

| Item | Value |
|---|---|
| Canvas resolution | 576 × 288 pixels per eye |
| Colour depth | 4-bit greyscale (16 levels) |
| Max containers per page | 4 |
| Max text content (startup/rebuild) | 1000 characters |
| Max text content (upgrade) | 2000 characters |
| Max list items | 20 |
| Max item name length | 64 characters |
| Max container name length | 16 characters |
| Image width range | 20–200 px |
| Image height range | 20–100 px |
| Border width range | 0–5 |
| Border radius range | 0–10 |
| Padding range | 0–32 |
| Border colour range (text) | 0–16 |
| Border colour range (list) | 0–15 |
| BLE range | ~28m |
| Audio: sample rate | 16kHz |
| Audio: frame length | 10ms |
| Audio: bytes per frame | 40 |
| Audio: encoding | PCM S16LE, mono |
| Full-screen text capacity | ~400–500 characters |
| Recommended scroll cooldown | 300ms |
| BT.601 luminance formula | `0.299R + 0.587G + 0.114B` |

---

## Appendix C: Known SDK Typos / Gotchas

| Issue | Detail |
|---|---|
| `borderRdaius` | Typo in protobuf definition, preserved in SDK. Use `borderRdaius` (not `borderRadius`) |
| `APP_REQUEST_REBUILD_PAGE_FAILD` | Internal error code uses "FAILD" (typo for "FAILED") |
| `CLICK_EVENT = 0` → `undefined` | SDK's `fromJson` normalises `0` to `undefined`. Always check for both |
| Missing `currentSelectItemIndex` at index 0 | Simulator (and sometimes hardware) omits it for the first item |
| Simulator sends `sysEvent` for clicks | Real hardware sends `textEvent` or `listEvent` — handle all three |
| Image tiling | Hardware tiles (repeats) image data if smaller than container — always match dimensions |
| No concurrent image sends | Queue image updates; wait for completion before sending next |
| `textContainerUpgrade` visual diff | Simulator causes full visual redraw; real hardware is smoother |
| `rebuildPageContainer` flicker | Full redraw on real hardware causes a brief flicker |

---

## Appendix D: Development Flow Summary

```
1. npm install @evenrealities/even_hub_sdk
2. Build web app (any framework)
3. Run locally: npm run dev (Vite on port 5173)
4. Generate QR: npx evenhub qr --url "http://<your-ip>:5173"
5. Scan QR with Even App on iPhone
6. App loads on glasses; Vite HMR works for live changes
7. For production: npm run build && npx evenhub pack app.json dist -o myapp.ehpk
```
